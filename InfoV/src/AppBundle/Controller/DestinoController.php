...
class DestinoController extends Controller
{
    ...
    public function indexAction()
    {
     ...
     public function destinosAction(Request $request)
    {
        $var=$request->query->get("search");
        $var=$request->query->get("search");
        $destino = $this->getDoctrine()->getRepository('AppBundle:Destino')->findAll();
        $asignacion = 0;
        foreach ($destino as $des){
            if ($des->getLugarDestino() == $var){
                $asignacion = 1;
            }
        }
        return $this->render('destino/destinos.html.twig', array(
            'destinos' => $destino,
            'id'=>$var,
